...
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  const payslip = await prisma.payslip.findFirst({
    ...
    include: {
      item: {
        include: {
          employee: { include: { teacher: { select: { userId: true } } } },
  ...
  const own =
    payslip.item.employee.userId === session.userId ||
    payslip.item.employee.teacher?.userId === session.userId;
  ...
  return pdfFileResponse(pdf, `payslip-${payslip.number}.pdf`);
}
